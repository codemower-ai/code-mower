...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument("--event-path", type=Path, help="Verified local event adapter output")
    args = parser.parse_args(argv)
    config = load_lane_config(args.lane)

    event_path = args.event_path or Path(os.environ["GITHUB_EVENT_PATH"])
    ...
    if not os.environ.get("DRY_RUN"):
        ...
        if issue_number and "pull_request" in issue:
            ...
            try:
                ...
                # A verified workflow verdict identifies the reviewer lane without
                # treating the local dispatcher's human login as that reviewer.
                # Explicit builder identity and verified contribution history remain.
                comment = event.get("comment") or {}
                publication_attested = (
                    (comment.get("user") or {}).get("login") == "github-actions[bot]"
                    and "<!-- CODE_MOWER_AUDIT_PUBLICATION: " in str(comment.get("body") or "")
                    and github_actions_comment_attested(repo=repo, body=comment.get("body") or "",
                        comment_id=comment.get("id"), issue_number=issue_number,
                        head_sha=current_head_sha,
                        workflow_paths=tuple(parse_csv_set(os.environ.get("CODE_MOWER_GITHUB_ACTIONS_WORKFLOWS") or "")),
                        tokens=tokens)
                )
                lineage = lineage_from_environment(repo, issue_number, pr_current,
                    lineage_core.History(issue_comments), reviewer=args.lane,
                    accounts=tuple(config.default_authors if publication_attested else config.comment_authors()))
