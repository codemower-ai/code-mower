...
Live operation requires the current immutable reviewed `code-mower==1.5.2`
...
Connect, public channels, DMs, and rich Slack UX are outside v1.5.2 and remain
planned v1.6 work.
...
1. Sign in to Code Mower, select the intended team, then open **Setup → Manage
   ...
   ID; v1.5.2 still rejects organization-wide installation.

2. Choose **Install**, review Slack's consent screen, and authorize the Code
   Mower app in that same workspace. The v1.5.2 hosted app requests only the bot
