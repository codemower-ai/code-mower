...
The current hosted value is private team signal. The network-effect value is a
...
`work_order` events are generated by
`code-mower work-order draft --issue-plan ...`. They let CodeMower.com connect
planning provenance to later builder/reviewer evidence: repository, GitHub
issue number/URL, role lenses, review lanes, and Code Mower package provenance.
They intentionally exclude issue bodies, source code, raw diffs, transcripts,
stdout/stderr, auth output, and secrets.

`board_snapshot` events are generated only by the explicit cloud Board command.
They summarize the native local Board for a hosted mirror without sending the
full local payload:

```bash
code-mower cloud board-snapshot --repo-slug OWNER/REPO --json
code-mower cloud board-snapshot --repo-slug OWNER/REPO --yes --json
```

The first command is a dry run. The second uploads one metadata-only event with
zero reports after inspection. The event keeps PR lane/check metadata, workflow
state, owner-queue reasons, agent card provider/status metadata, and aggregate
verdict/spend counts. It omits PR titles, owner-note titles, local cwd paths,
PIDs, full head SHAs, gate rerun commands, source, raw diffs, transcripts, issue
body text, raw stdout/stderr, auth output, browser history, local secret values,
and secrets.
...
The current cloud service starts with:
