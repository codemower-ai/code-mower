...
The current verified public beta baseline is `v0.5.0-beta.21`, published on
PyPI as `code-mower==0.5.0b21`. It is intended to be installed from the package
index for friendly-user pilots, with GitHub tag/source installs kept as a
fallback and development path. Beta.21 keeps the PyPI-first install path,
...
- Code Mower Cloud dogfood events from the OSS repo, CodeMower.com, and two
  private reference/product repos, with beta.21 preserving that client path for
...
- a package-installed beta.21 calibration/value-report pipeline target that
  keeps reviewer metrics, lane policy, value-report artifacts, and sanitized
  report upload in the release rehearsal path; and
- a beta.21 private-repo install rehearsal target against
  [DrinkBetter-AI/mobile-app](https://github.com/DrinkBetter-AI/mobile-app)
  to prove `code-mower==0.5.0b21` can detect and dry-run repository-native
  checks in an external-ish private repo without committing support files first;
  and
- a public PyPI package-install rehearsal from `code-mower==0.5.0b21` with a
...
- a local Codex dogfood/catch-up proof using a dashboard-issued token: beta.21
