...
| `devin_cli` | Devin CLI | local runner / doctor contract | included/provider account | local checkout plus local auth | informational; not merge authority until calibrated; not a local audit workflow selection until #746; selectable for release campaigns with the #744 maintained adapter |
...
| `devin` | Devin CLI on the self-hosted Mac lane runner, or a Devin session dispatched externally | `tier:R` plus `builder:devin`; `lane-mac-runner.yml` opens `devin/...` PRs, or external hosted dispatch/manual handoff then `builder record` | First-class local builder identity when run through the Mac lane runner: dedicated disposable checkout, dangerous permission mode (Devin's sandbox blocks on interactive confirmation), never `--export`. `devin_cloud` remains an accepted hosted alias. `builder:devin` remains the builder provenance identity for both paths. Codex and Claude audits gate before merge. |
