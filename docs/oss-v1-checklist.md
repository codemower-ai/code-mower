...
The current public-release baseline is `v0.5.0-beta.31` of the standalone
...
- public package installation from PyPI and TestPyPI as `code-mower==0.5.0b31`;
...
- production dogfood uploads from Code Mower OSS, CodeMower.com, and two
  private reference/product repos, with beta.31 preserving the same client path
...
- beta.31 local dogfood uploads from a stored dashboard-issued token, proving
...
- beta.31 private-repo package-install rehearsal against
...
- beta.31 calibration/value-report generation from the installed package plus
