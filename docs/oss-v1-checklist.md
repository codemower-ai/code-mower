...
The current public-release baseline is `v0.5.0-beta.15` of the standalone
...
- public package installation from PyPI as `code-mower==0.5.0b15`;
...
- production dogfood uploads from Code Mower OSS, CodeMower.com, and two
  private reference/product repos, with beta.15 preserving the same client path
...
- beta.15 private-repo package-install rehearsal against
...
- beta.15 calibration/value-report generation from the installed package plus
...
```bash
pipx install --python python3.12 code-mower==0.5.0b15
