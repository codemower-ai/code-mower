...
The current public-release baseline is `v0.5.0-beta.33` of the standalone
...
- public package installation from PyPI and TestPyPI as `code-mower==0.5.0b33`;
...
- production dogfood uploads from Code Mower OSS, CodeMower.com, and two
  private reference/product repos, with beta.33 preserving the same client path
...
- beta.33 local dogfood uploads from a stored dashboard-issued token, proving
...
- beta.33 private-repo package-install rehearsal against
...
- beta.33 calibration/value-report generation from the installed package plus
...
```bash
pipx install --python python3.12 code-mower==0.5.0b33
...
code-mower migration package-install-rehearsal \
  --package-spec code-mower==0.5.0b33 \
