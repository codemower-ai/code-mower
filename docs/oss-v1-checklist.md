...
The current public-release baseline is `v0.5.0-beta.32` of the standalone
...
- public package installation from PyPI and TestPyPI as `code-mower==0.5.0b32`;
...
- production dogfood uploads from Code Mower OSS, CodeMower.com, and two
  private reference/product repos, with beta.32 preserving the same client path
...
- beta.32 local dogfood uploads from a stored dashboard-issued token, proving
...
- beta.32 private-repo package-install rehearsal against
...
- beta.32 calibration/value-report generation from the installed package plus
