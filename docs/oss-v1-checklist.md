...
The current public-release baseline is `v0.5.0-beta.20` of the standalone
...
- public package installation from PyPI as `code-mower==0.5.0b20`;
...
- production dogfood uploads from Code Mower OSS, CodeMower.com, and two
  private reference/product repos, with beta.20 preserving the same client path
...
- beta.20 local dogfood and catch-up uploads from a stored dashboard-issued
  ...
  secrets;
- beta.20 private-repo package-install rehearsal against
...
- beta.20 calibration/value-report generation from the installed package plus
...
```bash
pipx install --python python3.12 code-mower==0.5.0b20
...
code-mower migration package-install-rehearsal \
  --package-spec code-mower==0.5.0b20 \
