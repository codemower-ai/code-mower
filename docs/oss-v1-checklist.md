...
The current public-release baseline is `v0.5.0-beta.38` of the standalone
...
- public package installation from PyPI and TestPyPI as `code-mower==0.5.0b38`;
...
- production dogfood uploads from Code Mower OSS, CodeMower.com, and two
  private reference/product repos, with beta.38 preserving the same client path
...
- beta.38 local dogfood uploads from a stored dashboard-issued token, proving
...
- beta.38 private-repo package-install rehearsal against
...
- beta.38 calibration/value-report generation from the installed package plus
...
- CodeMower.com planning-lineage drilldown for issue -> posted plan -> work
  order -> pull request -> reviewer checks -> merge -> upload chains;
...
code-mower migration package-install-rehearsal \
  --package-spec code-mower==0.5.0b38 \
