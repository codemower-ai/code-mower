...
this document is the current v1.1.2 supervised-pilot release.
...
pipx install --python "$CODE_MOWER_PYTHON" code-mower==1.1.2
```

The current package-index release baseline is `v1.1.2`, with pinned package install spec `code-mower==1.1.2`. Release evidence is recorded on the GitHub release and in the first-user install rehearsal.
...
The friendly-user loop is good enough for a wider supervised-pilot push when:
