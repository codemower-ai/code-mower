...
this document is the current v1.1.1 supervised-pilot release.
...
pipx install --python "$CODE_MOWER_PYTHON" code-mower==1.1.1
```

The current package-index release baseline is `v1.1.1`, with pinned package install spec `code-mower==1.1.1`. Release evidence is recorded on the GitHub release and in the first-user install rehearsal.
