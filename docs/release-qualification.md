...
Codex receives an API-compatible subset of the schema (without `if`/`then`/`else` conditionals, with `failure_reason` as a required nullable field) because the OpenAI structured-output endpoint rejects conditional keywords and optional object properties; the full closed local validator remains authoritative and strips `null` values before validating.
...
|---|---|---|
| `codex` | `codex` | `codex exec` with stdin (`-`), `--ephemeral`, `--approve-for-me`, a Code Mower-owned root-deny/workspace-write profile, an explicit keyring (default) or file credential store, `--skip-git-repo-check`, `--json`, `--output-schema`, `--output-last-message`, `-C` (codex-cli 0.147.0) |
...
Codex campaign runs use an isolated `CODEX_HOME` at `~/.config/code-mower/provider-homes/codex` (override with `CODE_MOWER_CODEX_CAMPAIGN_HOME`). Code Mower creates its non-secret restricted config automatically. The default keyring mode refuses a readable `auth.json`; explicit file mode requires that path to be a private regular file and refuses links or other file types. On a desktop-keyring host, authenticate the default home once with `CODEX_HOME="$HOME/.config/code-mower/provider-homes/codex" codex login --device-auth -c 'cli_auth_credentials_store="keyring"' --enable secret_auth_storage`; the explicit login flags make Codex store that home-specific credential in the OS keyring even before Code Mower has created the config file. The adapter preserves the real OS `HOME` only so the platform keyring can locate the user's login keychain; Codex configuration and state remain isolated under `CODEX_HOME`, ambient token variables are removed, and the root-deny policy lets the agent write only its disposable workspace. Network remains available for package installation. A previous result file is removed before every adapter attempt, and a failed run never leaves stale evidence for a caller to accept. Headless hosts can select the file-backed source below.
...
2. Create the isolated home's restricted configuration:
   ...
   as `missing` until step 3, and it writes that configuration for the selected
   mode first, including on a home carried over from keyring mode, so the login
   in step 3 stores the credential in the source you selected rather than in
   the store the home named before. This step does not create, read, or move a
   credential.
...
Claude Code 2.1.258 on macOS cannot complete pip's default certificate verification inside the maintained strict sandbox: pip 24.2+ verifies through the platform trust store, that path calls Security.framework, and the sandbox denies the call with `OSStatus -26276` even though `pypi.org` and `files.pythonhosted.org` are allowlisted. The same install succeeds outside the sandbox and on every other campaign adapter, so this is the upstream Claude Code macOS sandbox certificate-validation defect rather than a Code Mower index, network, or packaging problem.
...
- **Deterministic Python 3.12+ runtime resolution**: Before invoking local provider adapters, the campaign runner resolves a supported Python 3.12+ executable and passes exact `--python-bin` and `--target-runtime` arguments. An explicit `CODE_MOWER_PYTHON` remains authoritative. Automatic selection prefers versioned PATH runtimes outside the user home before falling back to Code Mower's running interpreter or another home-contained runtime; this lets a pipx- or uv-installed CLI use a Homebrew/system Python that remains readable inside Claude's strict macOS sandbox while preserving hosted environments that have no system runtime. Providers must not pick ambient `python3`. If no supported runtime exists, the dispatch fails closed with `python_runtime_unavailable` and actionable remediation. Result validators enforce `runtime_class >= python_3.12`.
...
**Campaign intent rule.** Release-campaign authentication is part of adoption doctor only when the run has campaign intent, decided by one provider-neutral rule reported as `doctor.campaign.intent`: the operator asked explicitly (`code-mower doctor --campaign`, which implies `--adoption`), `code-mower.yml` configures a campaign adapter for at least one lane, or `.code-mower/campaigns` holds a campaign whose status is not `complete`. Ordinary reviewer/orchestrator adoption of a repository with none of those reports every local provider's `doctor.campaign.auth` as a non-blocking `not_requested` skip, runs no login probe, and raises no owner action for an unused release capability. When intent is present, every selected campaign provider is checked with the authenticated/unauthenticated/unknown/skipped semantics above; Claude and Codex share those semantics.

**Headless Linux.** Codex defaults to the keyring-backed isolated campaign
home declared by `campaign_auth_keyring_required`, but v1.5.0 also supports the
explicit file-backed source described in
[Headless Linux campaign authentication](#headless-linux-campaign-authentication).
When a Codex probe confirms logged out on a Linux host with no desktop session
(`DISPLAY`/`WAYLAND_DISPLAY` absent) while keyring mode is selected, doctor says
so and recommends selecting `CODE_MOWER_CODEX_CAMPAIGN_AUTH_MODE=file` or using
one of the other supported routes: dispatch from a host with a desktop-session
keyring, run doctor with `--hosted-builders` or `--orchestrator-only`, or set
`CODE_MOWER_CAMPAIGN_AUTH_PROBE=0` to leave the lane capability-only. Doctor
reports the missing desktop session as a fact and never guesses whether a
headless keyring daemon happens to exist; an authenticated probe still passes
on any host. Claude's local adapter carries no Codex credential-store
requirement.
...
- **Credentials and issue binding:** an issue-comment transport dispatches by invoking `gh` itself, so when none of the lane's token variables (`CURSOR_CLOUD_AGENT_AUDIT_LABEL_TOKEN`, `GITHUB_TOKEN`) is exported, Code Mower falls back to a bounded, read-only `gh auth token` probe and treats an already authenticated CLI as valid credentials; the probe reports only success or failure, and the token itself is never printed, logged, returned, or persisted. Exported token variables keep strict precedence and skip the probe entirely, and the fallback is limited to these GitHub-comment transports -- Devin's API credential resolution is unchanged. The `--issue <number>` you create or first complete a campaign with is *bound* to that campaign, so dispatch, resume, watch, retry, and result discovery all reuse it without the flag; a later `--issue` naming a different issue is rejected before any comment is posted or campaign state is written, and a campaign stored before issue binding existed simply carries none and is completed once by the next `--issue`.
- The dispatch comment states exactly what will be accepted and includes a shell-quoted `code-mower release qualify` command carrying the campaign's exact release tag, package spec/source, qualification context, provider/executor, repository, and starting version when applicable. The provider embeds the complete generated adoption-result object unchanged in its bound reply rather than recreating that schema by hand. For an upgrade campaign the comment also carries the exact `starting_version` in the machine-readable `code_mower.releaseCampaignDispatch.v1` marker, so a remote runner never has to guess which starting version to qualify from. Cold-install (and `unknown`) campaigns have no starting version and omit the field. An upgrade campaign whose stored `starting_version` is missing is never dispatched at all: the provider stays `unavailable` with the bounded `campaign_identity_incomplete` error code and no comment is posted.
