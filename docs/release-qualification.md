...
**Headless Linux.** Codex is the one built-in provider whose isolated campaign home is keyring-only (`campaign_auth_keyring_required`); Claude's local adapter carries no such requirement. When a Codex probe confirms logged-out on a Linux host with no desktop session (`DISPLAY`/`WAYLAND_DISPLAY` absent), doctor says so and does not recommend the keyring login on that host. It offers the supported routes instead: dispatch Codex campaigns from a host with a desktop-session keyring, run doctor with `--hosted-builders` or `--orchestrator-only`, or set `CODE_MOWER_CAMPAIGN_AUTH_PROBE=0` to leave the lane capability-only. Doctor reports the missing desktop session as a fact and never guesses whether a headless keyring daemon happens to exist; an authenticated probe still passes on any host.
