...
class PublishedIdentityTests(unittest.TestCase):
    ...
    def test_shared_baseline_sentence_matches_the_published_version(self):
        ...
        self.assertIn("`v1.6.0`", sentence)
        self.assertIn("`code-mower==1.6.0`", sentence)
...
class PublicReleaseChecklistTests(unittest.TestCase):
    def test_checklist_names_v160_as_the_source_entrypoint(self):
        checklist = " ".join(_read("docs/public-release-checklist.md").split())
        self.assertIn(
            "The v1.6.0 source defines package-index entrypoint "
            "`code-mower==1.6.0` (GitHub tag `v1.6.0`). Confirm that the tag "
...
class VersionIdentityTests(unittest.TestCase):
    def test_source_version_is_1_6_0(self):
    ...
    def test_release_tag_for_current_version(self):
        self.assertEqual(release_readiness._release_tag_for_version(__version__), "v1.6.0")
...
class InstalledPromptPackTests(unittest.TestCase):
    def test_literal_starter_and_explicit_config_walkthrough(self):
        """Exercise installed 1.6.0 code, with no provider login or network doctor probes."""
...
assert code_mower.__version__ == '1.6.0'
