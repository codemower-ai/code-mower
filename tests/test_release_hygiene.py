...
class ReleaseHygieneTests(unittest.TestCase):
    ...
    def test_install_and_upgrade_docs_cover_agent_paths(self) -> None:
        ...
        self.assertIn("code-mower==1.0.13", troubleshooting)
...
""",
    ...
    def test_direct_cli_execution_points_to_package_or_dev_wrapper(self) -> None:
        ...
        self.assertNotEqual(completed.returncode, 0)
        self.assertIn("pipx install code-mower==1.0.13", completed.stderr)
...
"""
    ...
    def test_setup_drift_reports_standalone_pin_states(self) -> None:
        ...
        cases = (
            (
                'CODE_MOWER_STANDALONE_REF="v1.0.13"\n',
                "pass",
        ...
        for text, status, reason, current_ref in cases:
            ...
            self.assertEqual(payload["expected_ref"], "v1.0.13")
    ...
    def test_setup_drift_text_prints_absent_standalone_pin_line(self) -> None:
        ...
        self.assertIn("Standalone pin: SKIP pin_file_absent expected=v1.0.13", rendered)
...
""",
    ...
    def test_package_install_rehearsal_classifies_package_index_specs(self) -> None:
        ...
        self.assertFalse(
            code_mower_migration._package_spec_uses_package_index(
                "git+https://github.com/codemower-ai/code-mower.git@v1.0.13"
            )
        )
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(ValueError, "--allow-package-index"):
                code_mower_migration.run_package_install_rehearsal(
                    package_spec="code-mower==1.0.13",
                    work_dir=work_dir,
                )
    ...
    def test_release_readiness_reports_package_index_promotion_gate(self) -> None:
        ...
        self.assertEqual(payload["release_tag"], "v1.0.13")
        self.assertEqual(payload["alpha_tag"], "v1.0.13")
        self.assertEqual(payload["package_index_spec"], "code-mower==1.0.13")
        ...
        self.assertIn("--ref v1.0.13", commands["dry-run-release-workflow"])
        self.assertNotIn("--ref main", commands["dry-run-release-workflow"])
        self.assertIn("--ref v1.0.13", commands["publish-testpypi-candidate"])
    ...
    def test_release_readiness_fails_on_materialized_package_version_drift(
        ...
        self.assertEqual(check["detail"]["source_version"], "1.0.13")
    ...
    def test_release_readiness_tag_derivation_supports_release_stages(self) -> None:
        ...
        self.assertEqual(
            release_readiness._release_tag_for_version("1.0.13"),
            "v1.0.13",
        )
        self.assertEqual(
            code_mower_versioning.release_tag_for_version("1.0.13"),
            "v1.0.13",
        )
    ...
    def test_public_release_baseline_helpers_derive_announcement_links(self) -> None:
        self.assertEqual(
            ...
            (
                "The current package-index release baseline is `v1.0.13`, "
                "with pinned package install spec `code-mower==1.0.13`. "
        ...
        self.assertEqual(
            ...
            (
                ...
                "v1.0.13/docs/try-in-10-minutes.md"
            ),
        )
    ...
    def test_current_release_docs_record_package_index_procedure(self) -> None:
        ...
        current_status = (
            "The current package-index release baseline is `v1.0.13`, with "
            "pinned package install spec `code-mower==1.0.13`. Release "
        ...
        self.assertIn(
            "The current package-index release entrypoint is `code-mower==1.0.13`",
            public_release,
        )
        ...
        self.assertIn(
            "The current public-release baseline is `v1.0.13`",
            oss_checklist,
        )
        self.assertIn(
        ...
        self.assertEqual(first_user.count("  --ref v1.0.13 \\"), 2)
        ...
        self.assertNotIn(
            "The latest public-package rehearsal for `v1.0.13` was run",
            first_user,
        )
        self.assertNotIn("TestPyPI is not\npublished for `1.0.13`", first_user)
        for text in (readme, current_state, rollout):
            self.assertNotIn(
                "published on PyPI as `code-mower==1.0.13`",
                text,
            )
            self.assertNotIn(
    ...
    def test_install_docs_cover_supported_adoption_paths(self) -> None:
        ...
        self.assertIn("uv tool install --python 3.12 code-mower==1.0.13", install)
        self.assertIn(
    ...
    def test_active_docs_use_current_release_language(self) -> None:
        ...
        stale_phrases = (
            "pipx install --python python3.12 code-mower==1.0.13",
    ...
    def test_v09_adoption_polish_docs_cover_cold_start_surface(self) -> None:
        ...
        release_notes = (ROOT / "docs" / "v1013-release-notes.md").read_text(
            encoding="utf-8",
        )
        ...
        self.assertIn("Code Mower v1.0.13 Release Notes", release_notes)
    ...
    def test_next_steps_includes_cloud_upload_dry_run_after_export(self) -> None:
        ...
        self.assertIn("code-mower==1.0.13", package_step["command"])
