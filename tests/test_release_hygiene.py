...
class ReleaseHygieneTests(unittest.TestCase):
    ...
    def test_install_and_upgrade_docs_cover_agent_paths(self) -> None:
        ...
        self.assertIn("code-mower==1.5.1", troubleshooting)
...
""",
    ...
    def test_direct_cli_execution_points_to_package_or_dev_wrapper(self) -> None:
        ...
        self.assertNotEqual(completed.returncode, 0)
        self.assertIn("pipx install code-mower==1.5.1", completed.stderr)
...
""",
    ...
    def test_release_readiness_reports_package_index_promotion_gate(self) -> None:
        ...
        self.assertEqual(payload["release_tag"], "v1.5.1")
        self.assertEqual(payload["alpha_tag"], "v1.5.1")
        self.assertEqual(payload["package_index_spec"], "code-mower==1.5.1")
        ...
        self.assertEqual(manifest_check["detail"]["manifest_version"], "1.5.1")
        ...
        self.assertIn("--ref v1.5.1", commands["dry-run-release-workflow"])
        ...
        self.assertIn('--title "Code Mower v1.5.1"', release)
        self.assertIn('--notes-file "$GITHUB_RELEASE_NOTES"', release)
        self.assertIn('$CANDIDATE_DIR/code_mower-1.5.1-py3-none-any.whl', release)
        self.assertEqual(
    ...
    def test_release_readiness_fails_on_materialized_package_version_drift(
        ...
        self.assertEqual(check["status"], "fail")
        self.assertEqual(check["detail"]["source_version"], "1.5.1")
    ...
    def test_release_readiness_fails_on_committed_manifest_version_drift(self) -> None:
        ...
        self.assertEqual(check["detail"]["init_version"], "1.5.1")
    ...
    def test_current_runbook_does_not_authorize_hosted_devin_creation(self) -> None:
        runbook = self._runbook_section()
        self.assertIn("--required-providers claude,codex", runbook)
        self.assertNotIn("--required-providers claude,codex,devin", runbook)
        self.assertNotIn("DEVIN_PERMISSIONS_OWNER_CONFIRMED=", runbook)
        self.assertIn("No paid hosted Devin creation is authorized", runbook)
    ...
    def test_historical_runbook_retains_hosted_devin_readiness_checks(self) -> None:
        runbook = (ROOT / "docs/v140-release-runbook.md").read_text()
        required = {
    ...
    def test_runbook_campaign_gate_rejects_a_passing_duplicate_lane(self) -> None:
        for surface in ("watch", "status"):
            with self.subTest(surface=surface):
                ...
                if surface == "watch":
                    failing = dict(watch["providers"][1], state="error", error="boom")
                    watch["providers"] = [
                        watch["providers"][0],
                        watch["providers"][1],
                        failing,
                    ...
                    failing = dict(status["providers"][1], state="error", error="boom")
                    status["providers"] = [
                        status["providers"][0],
                        status["providers"][1],
                        failing,
    ...
    def test_runbook_campaign_gate_rejects_an_unbound_adoption_result(self) -> None:
        for field, value in (
            ...
            with self.subTest(field=field):
                status = self._campaign_status_payload()
                status["providers"][1]["adoption_result"][field] = value
                ...
                self.assertEqual(result.returncode, 1)
                self.assertIn("codex lane result", result.stderr)
    ...
    def test_public_release_baseline_helpers_derive_announcement_links(self) -> None:
        self.assertEqual(
            ...
            (
                ...
                "`code-mower==1.5.1`. Confirm the release tag on GitHub Releases "
        ...
        self.assertEqual(
            ...
            (
                ...
                "v1.5.1/docs/try-in-10-minutes.md"
            ),
        )
    ...
    def test_public_docs_match_current_commands_and_privacy_boundary(self) -> None:
        ...
        self.assertIn("included in `code-mower==1.5.1`", sessions)
    ...
    def test_current_release_docs_record_package_index_procedure(self) -> None:
        ...
        self.assertIn("The v1.5.1 supervised-pilot source includes", public_release)
        self.assertIn(
    ...
    def test_install_docs_cover_supported_adoption_paths(self) -> None:
        ...
        self.assertIn("uv tool install --python 3.12 code-mower==1.5.1", install)
        self.assertIn(
    ...
    def test_next_steps_includes_cloud_upload_dry_run_after_export(self) -> None:
        ...
        self.assertIn("code-mower==1.5.1", package_step["command"])
