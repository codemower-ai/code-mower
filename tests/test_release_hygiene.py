...
class ReleaseHygieneTests(unittest.TestCase):
    ...
    def test_install_and_upgrade_docs_cover_agent_paths(self) -> None:
        ...
        self.assertIn("code-mower==0.9.3b1", troubleshooting)
    ...
    def test_cli_default_help_is_first_user_focused(self) -> None:
        ...
        self.assertIn(
            (
                ...
                "code-mower==0.9.3b1 --allow-package-index --json"
            ),
            help_text,
        )
...
""",
    ...
    def test_direct_cli_execution_points_to_package_or_dev_wrapper(self) -> None:
        ...
        self.assertNotEqual(completed.returncode, 0)
        self.assertIn("pipx install code-mower==0.9.3b1", completed.stderr)
...
"""
    ...
    def test_setup_drift_reports_standalone_pin_states(self) -> None:
        ...
        cases = (
            (
                'CODE_MOWER_STANDALONE_REF="v0.9.3-beta.1"\n',
                "pass",
            ...
            with self.subTest(reason=reason):
                with tempfile.TemporaryDirectory() as tmp:
                    ...
                    payload = migration._standalone_pin_drift_summary(
                        repo,
                        package_version="0.9.3b1",
                    )

            self.assertEqual(payload["status"], status)
            self.assertEqual(payload["reason"], reason)
            self.assertEqual(payload["expected_ref"], "v0.9.3-beta.1")
...
""",
    ...
    def test_package_install_rehearsal_classifies_package_index_specs(self) -> None:
        ...
        self.assertFalse(
            code_mower_migration._package_spec_uses_package_index(
                "git+https://github.com/codemower-ai/code-mower.git@v0.9.3-beta.1"
            )
        )
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(ValueError, "--allow-package-index"):
                code_mower_migration.run_package_install_rehearsal(
                    package_spec="code-mower==0.9.3b1",
                    work_dir=work_dir,
                )
    ...
    def test_release_readiness_reports_package_index_promotion_gate(self) -> None:
        ...
        self.assertEqual(payload["status"], "pass")
        self.assertEqual(payload["version"], "0.9.3b1")
        self.assertEqual(payload["release_tag"], "v0.9.3-beta.1")
        self.assertEqual(payload["alpha_tag"], "v0.9.3-beta.1")
        self.assertEqual(payload["package_index_spec"], "code-mower==0.9.3b1")
        ...
        self.assertIn("--ref v0.9.3-beta.1", commands["dry-run-release-workflow"])
        self.assertNotIn("--ref main", commands["dry-run-release-workflow"])
        self.assertIn("--ref v0.9.3-beta.1", commands["publish-testpypi-candidate"])
    ...
    def test_release_readiness_fails_on_materialized_package_version_drift(
        ...
        self.assertEqual(check["detail"]["source_version"], "0.9.3b1")
    ...
    def test_public_release_baseline_helpers_derive_announcement_links(self) -> None:
        self.assertEqual(
            ...
            (
                "The current package-index beta baseline is `v0.9.3-beta.1`, "
                "with pinned package install spec `code-mower==0.9.3b1`. "
        ...
        self.assertEqual(
            ...
            (
                ...
                "v0.9.3-beta.1/docs/try-in-10-minutes.md"
            ),
        )
    ...
    def test_current_beta_release_docs_record_package_index_procedure(self) -> None:
        ...
        current_status = (
            "The current package-index beta baseline is `v0.9.3-beta.1`, with "
            "pinned package install spec `code-mower==0.9.3b1`. Release "
        ...
        self.assertIn(
            "The current package-index beta entrypoint is `code-mower==0.9.3b1`",
            public_release,
        )
        ...
        self.assertIn(
            "The current public-release baseline is `v0.9.3-beta.1`",
            oss_checklist,
        )
        self.assertIn(
        ...
        self.assertEqual(first_user.count("  --ref v0.9.3-beta.1 \\"), 2)
        ...
        self.assertNotIn(
            "The latest public-package rehearsal for `v0.9.3-beta.1` was run",
            first_user,
        )
        self.assertNotIn("TestPyPI is not\npublished for `0.9.3b1`", first_user)
        for text in (readme, current_state, rollout):
            self.assertNotIn(
                "published on PyPI as `code-mower==0.9.3b1`",
                text,
            )
            self.assertNotIn(
    ...
    def test_install_docs_cover_supported_adoption_paths(self) -> None:
        ...
        self.assertIn("uv tool install --python 3.12 code-mower==0.9.3b1", install)
        self.assertIn(
    ...
    def test_active_docs_use_current_beta_release_language(self) -> None:
        ...
        stale_phrases = (
            "pipx install --python python3.12 code-mower==0.9.3b1",
    ...
    def test_v09_adoption_polish_docs_cover_cold_start_surface(self) -> None:
        ...
        self.assertIn("Code Mower v0.9.3-beta.1 Release Notes", release_notes)
    ...
    def test_next_steps_includes_cloud_upload_dry_run_after_export(self) -> None:
        ...
        self.assertIn("code-mower==0.9.3b1", package_step["command"])
