...
class BoardObservationTests(unittest.TestCase):
    def test_valid_fixtures_cover_frozen_truth_states(self) -> None:
        expected = {
            "assigned",
            "dispatched",
            "observed_running",
    ...
    def test_adversarial_fixtures_fail_closed_with_bounded_diagnostics(self) -> None:
        for case in fixture()["adversarial"]:
            record = apply_fixture_values(named_record(case["base"]), case["set"])
            with self.assertRaisesRegex(BoardObservationError, f"^{case['error']}$"):
                validate(record)

    def test_multiple_reasons_have_one_canonical_primary_route(self) -> None:
    ...
    def test_reason_duplicates_and_noncanonical_order_fail(self) -> None:
        with self.assertRaisesRegex(BoardObservationError, "invalid_reason"):
        ...
        with self.assertRaisesRegex(BoardObservationError, "invalid_route"):
            validate(record)

    def test_provider_lifecycle_is_exactly_the_existing_public_projection(self) -> None:
    ...
    def test_request_label_lease_and_gate_publisher_do_not_claim_execution_or_gate(self) -> None:
    ...
    def test_unavailable_source_preserves_last_observation_without_live_claim(self) -> None:
    ...
    def test_no_work_requires_fresh_complete_session_queue_and_run_coverage(self) -> None:
        record = copy.deepcopy(named_record("no_work"))
        assert validate(record)["kind"] == "no_work"

        record["sources"][2]["coverage"] = "partial"
        with self.assertRaisesRegex(BoardObservationError, "insufficient_coverage"):
            validate(record)

    def test_unknown_measurement_is_null_not_zero_or_success(self) -> None:
    ...
    def test_authorized_session_label_is_explicit_and_bounded(self) -> None:
        record = copy.deepcopy(named_record("no_work"))
        assert validate(record)["display"]["session_label"] == "Board contract delivery"

        record["display"]["session_label"] = "/private/example"
        with self.assertRaisesRegex(BoardObservationError, "privacy_violation"):
            validate(record)

    def test_decode_is_bounded_duplicate_safe_and_finite(self) -> None:
    ...
    def test_validation_returns_a_detached_copy(self) -> None:
        record = named_record("observed_running")
        result = validate(record)
        result["sources"][0]["freshness"] = "stale"
        assert record["sources"][0]["freshness"] == "fresh"

    def test_schema_is_closed_and_constants_do_not_drift(self) -> None:
        contract = schema()
    ...
    def test_packaged_contract_files_are_declared(self) -> None:
