...
class JiraDoctorAdoptionTests(unittest.TestCase):
    ...
    def test_doctor_comment_requires_edit_permission_for_replay_claim(self) -> None:
        cfg = sample_jira_config(
            mutations={
                "writes_enabled": False,
                "allowed_operations": ["comment"],
                "transitions": {},
            },
        )
        permissions = {
    ...
    def test_doctor_reports_unverified_transition(self) -> None:
        cfg = sample_jira_config(
            status_category_map={"in_progress": ["10001"]},
            mutations={
                "writes_enabled": False,
                "allowed_operations": ["transition"],
                "transitions": {"in_progress": "999999"},
            },
        )
        permissions = {
            "globalPermissions": [],
        ...
        self.assertEqual(mutation_check.status, "warn")
        self.assertEqual(
            mutation_check.detail.get("reason"), "transition_unverified"
        )
        self.assertEqual(
            mutation_check.detail.get("unverified_transitions"),
            [{"category": "in_progress", "transition_id": "999999"}],
        )

        cfg["tracker"]["jira_cloud"]["mutations"]["writes_enabled"] = True
        runner = FakeHttp(
            [
                http_response(load_fixture("server_info.json")),
                http_response(load_fixture("project.json")),
    ...
    def test_doctor_verifies_transitions_across_sampled_issue_states(self) -> None:
        cfg = sample_jira_config(
            status_category_map={"in_progress": ["10001"]},
            mutations={
                "writes_enabled": True,
        ...
        first = load_fixture("search_page2.json")["issues"][0]
        second = json.loads(json.dumps(first))
        second.update({"id": "20004", "key": "ABC-4"})
        search = {"isLast": True, "issues": [first, second]}
        runner = FakeHttp(
            [
                http_response(load_fixture("server_info.json")),
                http_response(load_fixture("project.json")),
                ...
                http_response(load_fixture("status_categories.json")),
                http_response(permissions),
                http_response(search),
                http_response(
                    {
                        "transitions": [
                            {"id": "11", "name": "Start", "to": {"id": "10000"}}
                        ]
                    }
                ),
                http_response(load_fixture("transitions.json")),
        ...
        self.assertEqual(mutation_check.status, "pass")
