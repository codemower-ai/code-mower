...
def context_main(argv: list[str] | None = None) -> int:
    ...
    if effective_argv[:1] == ['doctor']:
        from .context_readiness import main
    ...
    if effective_argv[:1] == ["fetch"]:
        ...
        return main(effective_argv[1:])
    if effective_argv[:1] in (["connect"], ["verify"], ["status"], ["disconnect"], ["identity"]):
    ...
    subparsers = parser.add_subparsers(dest="command", required=True)
    for name in ("connect", "verify", "status", "disconnect", "identity", "doctor", "fetch", "attach", "deliver", "feedback"):
