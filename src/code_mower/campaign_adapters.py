...
def build_qualification_prompt(
    *,
    ...
    lines.extend(
        ...
        "{id, status, elapsed_seconds, warning_count, owner_action_count} with an",
        "optional failure_reason on a failed package_install step and nowhere else.",
        "A failed package_install step must carry exactly one failure_reason:",
        "network for connection or TLS failures (connection refused, timeout,",
        "DNS, SSL or certificate validation), package_index for a missing version",
        "or 404 from the package index, runtime for Python or dependency",
        "incompatibility (unsupported Python version, missing module, conflicting",
        "dependencies), sandbox_permission for permission, disk, or sandbox",
        "denials (permission denied, disk full, read-only filesystem). Use",
        "unknown only for a command that was actually attempted and cannot be",
        "classified from local diagnostics. Report only the closed reason word;",
        "never include raw output, logs, paths, commands, source, diffs,",
        "transcripts, or secrets. Step id is",
        "one of board, doctor, lanes_status, overhead, package_install or a namespaced",
