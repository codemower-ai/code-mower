"""CodeMower.com client internals."""

from .bundle import (
    BUNDLE_MANIFEST_FILENAME,
    BUNDLE_SCHEMA,
    EXCLUDED_CONTENT,
    EXPECTED_BUNDLE_ENTRIES,
    MAX_EVENT_COUNT,
    MAX_REPORT_UPLOAD_BYTES,
    SAFE_EVENT_TYPES,
    SAFE_REPORT_KINDS,
    is_bundle_manifest,
    validate_metadata_payload,
)
from .endpoints import (
    DEFAULT_DASHBOARD_PATH,
    DEFAULT_HEALTH_PATH,
    dashboard_url_for_endpoint,
    health_url_for_endpoint,
    is_local_http_endpoint,
    probe_cloud_service,
    validate_upload_endpoint,
)
from .export import (
    build_provenance_summary,
    build_cloud_bundle,
    existing_bundle_manifest,
    plan_report,
    render_bundle_readme,
    safe_target_name,
    stage_reports,
    swap_reports,
    unexpected_bundle_entries,
)
from .adoption_runs import (
    ADOPTION_CONTEXTS,
    ADOPTION_COVERAGE_VALUES,
    ADOPTION_EXECUTION_STATES,
    ADOPTION_HOST_CLASSES,
    ADOPTION_OUTCOMES,
    ADOPTION_PROVIDER_POSTURES,
    ADOPTION_RESULT_SCHEMA,
    ADOPTION_RUN_COUNT_METRICS,
    ADOPTION_RUN_DIMENSIONS,
    ADOPTION_RUN_EVENT_TYPE,
    ADOPTION_RUN_SCHEMA,
    ADOPTION_RUN_TIME_METRICS,
    ADOPTION_STEP_STATUSES,
    adoption_event_from_result_dict,
    adoption_result_to_event,
    validate_adoption_run_payload,
)
from .dogfood import (
    DEFAULT_DOGFOOD_REPORTS,
    DogfoodPlan,
    build_dogfood_dry_run_preview,
    build_dogfood_plan,
    default_dogfood_reports,
)
from .doctor import (
    render_cloud_doctor_text,
    run_cloud_doctor,
)
from .events import (
    EVENT_SCHEMA,
    GITHUB_RUN_LIST_FIELDS,
    GITHUB_PR_LIST_FIELDS,
    artifact_event_from_dict,
    build_board_snapshot_event,
    build_dogfood_event,
    build_provider_catalog_snapshot_events,
    build_workflow_run_event,
    event_id_from_github_run,
    load_event_file,
    normalize_event,
    parse_event_args,
    parse_event_file_candidates,
    run_gh_pr_list,
    run_gh_run_list,
    safe_event_type,
    safe_kind,
    utc_now,
    validate_cloud_event,
)
from .git_metadata import (
    detect_repo_slug,
    repo_slug_from_remote,
    run_git,
)
from .manifest import (
    load_bundle_manifest,
    report_path_from_manifest,
)
from .reports import included_report_payloads
from .operations import (
    PRODUCTIVITY_BASELINE_TRUST_GUIDANCE,
    board_snapshot_upload,
    catch_up_upload,
    dogfood_upload,
    parse_repo_sync_spec,
    pr_outcomes_upload,
    repo_sync_output_name,
    repo_sync_upload,
    repo_sync_window_events,
    reviewer_runs_upload,
)
from .errors import CloudBundleError
from .setup import (
    DEFAULT_INSTALL_ID_ENV,
    DEFAULT_SETUP_INSTALL_ID,
    DEFAULT_TEAM_ID_ENV,
    DEFAULT_TOKEN_ENV,
    DEFAULT_UPLOAD_ENDPOINT,
    default_setup_path,
    read_token_file,
    render_setup_env,
    resolve_setup_token,
    run_cloud_setup,
    safe_config_stem,
    token_prefix,
    write_setup_env_file,
)
from .tokens import (
    CURRENT_PROFILE_FILENAME,
    DEFAULT_TOKEN_DIR,
    CloudTokenResolution,
    default_token_dir,
    display_token_path,
    read_token_profile,
    require_upload_token,
    resolve_cloud_endpoint,
    resolve_cloud_identity,
    resolve_cloud_token,
    write_current_token_profile,
)
from .productivity import (
    PRODUCTIVITY_AGGREGATION_SUBJECTS,
    PRODUCTIVITY_COUNT_METRICS,
    PRODUCTIVITY_EVENT_TYPE,
    PRODUCTIVITY_METRIC_UNITS,
    PRODUCTIVITY_METRICS_SCHEMA,
    PRODUCTIVITY_REQUIRED_DIMENSIONS,
    PRODUCTIVITY_TIME_METRICS,
    PRODUCTIVITY_TOKEN_METRICS,
    PRODUCTIVITY_WINDOW_GRANULARITIES,
    validate_productivity_summary_payload,
)
from .productivity_windows import (
    COMPARISON_BASIS_VALUES,
    PRODUCTIVITY_WINDOW_DIMENSION,
    PRODUCTIVITY_WINDOW_INPUT_SCHEMA,
    TIMING_PROVENANCE_VALUES,
    WINDOW_COUNT_FIELDS,
    WINDOW_SUBJECTS,
    WINDOW_TIMING_FIELDS,
    count_normalized_productivity_window_events,
    is_normalized_productivity_window_event,
    load_productivity_window_events,
    normalize_window_observation,
    productivity_window_event_from_dict,
    productivity_window_event_id,
    productivity_window_to_event,
    validate_productivity_window_event,
)
from .pr_outcomes import (
    PR_COST_COVERAGE_VALUES,
    PR_OUTCOME_COST_METRICS,
    PR_OUTCOME_COUNT_METRICS,
    PR_OUTCOME_DIMENSIONS,
    PR_OUTCOME_EVENT_TYPE,
    PR_OUTCOME_SCHEMA,
    PR_OUTCOME_VALUES,
    build_pr_outcome_event,
    load_pr_outcome_observations,
    max_source_freshness,
    pr_outcome_observation_key,
    pr_outcome_observation_record,
    save_pr_outcome_observations,
    validate_pr_outcome_payload,
)
from .upload import (
    UPLOAD_SCHEMA,
    build_event_upload_payload,
    build_upload_payload,
    build_upload_payload_from_manifest,
    post_upload_payload,
)
from .work_types import (
    WORK_TYPE_SCHEMA,
    WORK_TYPE_SUPPORTED_EVENT_TYPES,
    WORK_TYPE_VALUES,
    resolve_work_type_classification,
    validate_work_type_metadata,
)

__all__ = [
    "ADOPTION_CONTEXTS",
    "ADOPTION_COVERAGE_VALUES",
    "ADOPTION_EXECUTION_STATES",
    "ADOPTION_HOST_CLASSES",
    "ADOPTION_OUTCOMES",
    "ADOPTION_PROVIDER_POSTURES",
    "ADOPTION_RESULT_SCHEMA",
    "ADOPTION_RUN_COUNT_METRICS",
    "ADOPTION_RUN_DIMENSIONS",
    "ADOPTION_RUN_EVENT_TYPE",
    "ADOPTION_RUN_SCHEMA",
    "ADOPTION_RUN_TIME_METRICS",
    "ADOPTION_STEP_STATUSES",
    "BUNDLE_MANIFEST_FILENAME",
    "BUNDLE_SCHEMA",
    "DEFAULT_DASHBOARD_PATH",
    "DEFAULT_HEALTH_PATH",
    "EXCLUDED_CONTENT",
    "EXPECTED_BUNDLE_ENTRIES",
    "DEFAULT_DOGFOOD_REPORTS",
    "MAX_EVENT_COUNT",
    "MAX_REPORT_UPLOAD_BYTES",
    "COMPARISON_BASIS_VALUES",
    "PRODUCTIVITY_AGGREGATION_SUBJECTS",
    "PRODUCTIVITY_COUNT_METRICS",
    "PRODUCTIVITY_EVENT_TYPE",
    "PRODUCTIVITY_WINDOW_DIMENSION",
    "PRODUCTIVITY_WINDOW_INPUT_SCHEMA",
    "PRODUCTIVITY_METRIC_UNITS",
    "PRODUCTIVITY_METRICS_SCHEMA",
    "PRODUCTIVITY_REQUIRED_DIMENSIONS",
    "PRODUCTIVITY_TIME_METRICS",
    "PRODUCTIVITY_TOKEN_METRICS",
    "PRODUCTIVITY_WINDOW_GRANULARITIES",
    "TIMING_PROVENANCE_VALUES",
    "WINDOW_COUNT_FIELDS",
    "WINDOW_SUBJECTS",
    "WINDOW_TIMING_FIELDS",
    "PR_COST_COVERAGE_VALUES",
    "PR_OUTCOME_COST_METRICS",
    "PR_OUTCOME_COUNT_METRICS",
    "PR_OUTCOME_DIMENSIONS",
    "PR_OUTCOME_EVENT_TYPE",
    "PR_OUTCOME_SCHEMA",
    "PR_OUTCOME_VALUES",
    "build_pr_outcome_event",
    "load_pr_outcome_observations",
    "max_source_freshness",
    "pr_outcome_observation_key",
    "pr_outcome_observation_record",
    "save_pr_outcome_observations",
    "WORK_TYPE_SCHEMA",
    "WORK_TYPE_SUPPORTED_EVENT_TYPES",
    "WORK_TYPE_VALUES",
    "SAFE_EVENT_TYPES",
    "SAFE_REPORT_KINDS",
    "UPLOAD_SCHEMA",
    "CloudBundleError",
    "CloudTokenResolution",
    "CURRENT_PROFILE_FILENAME",
    "DEFAULT_INSTALL_ID_ENV",
    "DEFAULT_SETUP_INSTALL_ID",
    "DEFAULT_TEAM_ID_ENV",
    "DEFAULT_TOKEN_ENV",
    "DEFAULT_TOKEN_DIR",
    "DEFAULT_UPLOAD_ENDPOINT",
    "DogfoodPlan",
    "EVENT_SCHEMA",
    "GITHUB_RUN_LIST_FIELDS",
    "GITHUB_PR_LIST_FIELDS",
    "adoption_event_from_result_dict",
    "adoption_result_to_event",
    "artifact_event_from_dict",
    "build_dogfood_event",
    "build_dogfood_dry_run_preview",
    "build_dogfood_plan",
    "build_provider_catalog_snapshot_events",
    "build_workflow_run_event",
    "count_normalized_productivity_window_events",
    "is_normalized_productivity_window_event",
    "load_productivity_window_events",
    "normalize_window_observation",
    "productivity_window_event_from_dict",
    "productivity_window_event_id",
    "productivity_window_to_event",
    "validate_productivity_window_event",
    "build_cloud_bundle",
    "build_event_upload_payload",
    "build_upload_payload",
    "build_upload_payload_from_manifest",
    "build_board_snapshot_event",
    "build_provenance_summary",
    "board_snapshot_upload",
    "catch_up_upload",
    "PRODUCTIVITY_BASELINE_TRUST_GUIDANCE",
    "dashboard_url_for_endpoint",
    "default_setup_path",
    "default_token_dir",
    "detect_repo_slug",
    "display_token_path",
    "default_dogfood_reports",
    "dogfood_upload",
    "event_id_from_github_run",
    "existing_bundle_manifest",
    "health_url_for_endpoint",
    "included_report_payloads",
    "is_local_http_endpoint",
    "is_bundle_manifest",
    "load_bundle_manifest",
    "load_event_file",
    "normalize_event",
    "plan_report",
    "parse_repo_sync_spec",
    "repo_sync_window_events",
    "parse_event_args",
    "parse_event_file_candidates",
    "post_upload_payload",
    "probe_cloud_service",
    "read_token_file",
    "read_token_profile",
    "render_bundle_readme",
    "render_cloud_doctor_text",
    "render_setup_env",
    "repo_slug_from_remote",
    "pr_outcomes_upload",
    "repo_sync_output_name",
    "repo_sync_upload",
    "report_path_from_manifest",
    "require_upload_token",
    "resolve_cloud_endpoint",
    "resolve_cloud_identity",
    "resolve_cloud_token",
    "resolve_setup_token",
    "reviewer_runs_upload",
    "resolve_work_type_classification",
    "run_cloud_doctor",
    "run_cloud_setup",
    "run_gh_pr_list",
    "run_gh_run_list",
    "run_git",
    "safe_target_name",
    "safe_config_stem",
    "safe_event_type",
    "safe_kind",
    "stage_reports",
    "swap_reports",
    "token_prefix",
    "unexpected_bundle_entries",
    "utc_now",
    "validate_adoption_run_payload",
    "validate_cloud_event",
    "validate_pr_outcome_payload",
    "validate_metadata_payload",
    "validate_productivity_summary_payload",
    "validate_upload_endpoint",
    "validate_work_type_metadata",
    "write_setup_env_file",
    "write_current_token_profile",
]
