...
published `code-mower==1.3.1` package, use the documentation from the matching
[`v1.3.1` tag](https://github.com/codemower-ai/code-mower/tree/v1.3.1) so the
...
plus `.code-mower/reviewer-spend.json` when those files exist. When
`code-mower.yml` is present, the Board also embeds a supervised-pilot controller
summary so the browser view shows the current decision, queue counts, reviewer
evidence, selected PR or issue, and next action from the same metadata-only
policy engine as `code-mower controller run`. Local agents can
...
signals, provider scorecards, promotion caveats, fix rounds, owner actions,
cost/tokens, and merge/cycle metrics where known. Missing fields mean unknown,
not zero, and scorecard promotion recommendations stay advisory until reviewed
against [Lane Promotion Policy](docs/lane-promotion-policy.md).
...
[Try Code Mower In 10 Minutes](https://github.com/codemower-ai/code-mower/blob/v1.3.1/docs/try-in-10-minutes.md)
...
fixtures, and the universal orchestrator prompt pack. The v1.0.1 patch adds
local productivity reports, Board productivity summaries, provider scorecards,
CodeMower.com productivity views, and Board multi-instance administration. The
v1.0.2 patch makes release-smoke package-index checks more tolerant of fresh
PyPI/TestPyPI propagation lag and smooths hosted-agent adoption diagnostics.
The v1.0.3 patch makes Board status requests millisecond-fast after startup,
tightens stale-agent cleanup and config diagnostics, and refreshes the public
configuration and documentation for a clean senior-engineer adoption path.
The v1.0.4 patch makes the offline package rehearsal deterministic, clarifies
configuration provenance and hosted-agent diagnostics, hardens token-bearing
generated workflows, and records a fresh end-to-end installation-doc audit.
The v1.0.5 patch adds repeatable release qualification across provider
environments, resumable campaigns on Board, and an opt-in metadata-only
`adoption_run` view on CodeMower.com. Qualification proves install and
operational compatibility; it is not builder-quality or reviewer-promotion
evidence.
The v1.0.6 patch turns that campaign contract into an operator loop: maintained
Codex, Claude, Antigravity, and Muse adapters; bounded Cursor/Grok Bot and Devin
hosted profiles; a blocking `watch`; dry-run-first campaign upload; and adoption
doctor checks that identify provider, storage, cloud, and Board readiness.
The v1.0.7 patch hardens that loop from its first applied campaign: isolated
Codex auth works with the macOS keychain, Board distinguishes active from stale
local adapters, provider auth and hosted transport readiness are explicit, and
adoption results have stricter comparable-evidence semantics.
The v1.0.8 patch closes the remaining release-campaign trust gaps: audit
transport preserves valid metadata-only P3 findings, retries retain their full
chronology, local providers prove the requested Python runtime and real result
artifact, campaigns survive checkout changes, TestPyPI qualification is
closed, Cursor builder identity is distinct from Grok review, Antigravity uses
explicit project isolation, and hosted Cursor and Devin profiles require
verified dispatch and result transport.
The v1.0.9 patch makes Board refresh recovery deterministic, turns package
installation failures into bounded actionable diagnostics, and lets release
campaigns require established providers while retaining slower or experimental
providers as informational evidence.
The v1.0.10 patch makes local builder lanes deliverable and recoverable: a
provider-neutral delivery contract with bounded no-change and owner-action
outcomes, process-group cleanup so a finished provider is not reported as a
timeout, simpler first-run defaults, host-led participant sessions, and a
published 20-sample provider calibration scorecard.
The v1.0.11 patch moves canonical hosted Devin release campaigns onto the
bounded, pollable Sessions API transport and keeps macOS Claude release
qualification inside the maintained strict sandbox by using pip's
TLS-verifying certifi certificate path.
The v1.0.12 patch completes that Claude qualification fix: the maintained
provider qualification prompt no longer tells the agent to create its own
temporary directory, so it stays in the disposable workspace Code Mower
already created, and it states the existing contract that a result's total
`elapsed_seconds` is the sum of its step values.
The v1.0.13 patch makes automatic campaign runtime selection sandbox-safe:
when `CODE_MOWER_PYTHON` is not set, Code Mower now prefers a supported
versioned Python on `PATH` outside the user home before falling back to its
...
The v1.2 release gives every orchestrator host the same Jira authority brief,
adds a local single-orchestrator session lease with Board visibility, records
controller orchestrator telemetry, qualifies Cursor against the Codex/Claude
orchestrator contract, and makes hosted release qualification reproduce the
validated local result command exactly.
...
the package-index procedure for `v1.3.1` / `code-mower==1.3.1`. The
...
- [v1.0 Release Notes](docs/v10-release-notes.md)
- [v1.0.1 Release Notes](docs/v101-release-notes.md)
- [v1.0.2 Release Notes](docs/v102-release-notes.md)
- [v1.0.3 Release Notes](docs/v103-release-notes.md)
- [v1.0.4 Release Notes](docs/v104-release-notes.md)
- [v1.0.5 Release Notes](docs/v105-release-notes.md)
- [v1.0.6 Release Notes](docs/v106-release-notes.md)
- [v1.0.7 Release Notes](docs/v107-release-notes.md)
- [v1.0.8 Release Notes](docs/v108-release-notes.md)
- [v1.0.9 Release Notes](docs/v109-release-notes.md)
- [v1.0.10 Release Notes](docs/v1010-release-notes.md)
...
- [v1.3.1 Guided Context Release Notes](docs/v131-release-notes.md)
- [v1.3.1 Guided Context Qualification](docs/v131-guided-context-qualification.md)
