...
[Try Code Mower In 10 Minutes](https://github.com/codemower-ai/code-mower/blob/v0.9.1-beta.1/docs/try-in-10-minutes.md)
guide. The v0.9 beta includes the completed native Board work plus the adoption
and upgrade hardening from recent install rehearsals; v0.9.1 adds a smaller
announcement-hardening pass on Board visibility, doctor guidance, and safer
setup docs. Package-only users can start from the public package rather than a
source checkout.
...
the package-index procedure for `v0.9.1-beta.1` / `code-mower==0.9.1b1`. The
